...
bool _update_alarms(__alarm_info_t *__alarm_info)
{
	...
	char *query = sqlite3_mprintf("update alarmmgr set start=%d, end=%d,\
			...
			bundle=%Q, noti=%Q, year=%d, month=%d, day=%d, hour=%d, min=%d, sec=%d,\
