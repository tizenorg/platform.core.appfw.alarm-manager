		...
		alarm_id_t alarm_id, uid_t uid)
{
	...
	snprintf(saddr.sun_path, 100, "/run/alarm_agent/%d", uid);
	...
	ALARM_MGR_EXCEPTION_PRINT( "%s", service_name);
	memcpy(data, &len, 4);
	memcpy(data + 4, gv_data, len);
	ALARM_MGR_EXCEPTION_PRINT( "%s", data + 8);

	if (send(fd, data, len + 8, 0) == -1) {
		ALARM_MGR_EXCEPTION_PRINT( "sendto() failed - %d (errno %d)", len, errno);
		return FALSE;
	}
...
static void __alarm_send_noti_to_application(const char *app_service_name, alarm_id_t alarm_id, uid_t uid)
{
	...
	if (ret != TRUE) {
		ALARM_MGR_EXCEPTION_PRINT("failed to send expired signal for %d, %s: %s", alarm_id, service_name);
	}
}
	...
	for (iter = g_scheduled_alarm_list; iter != NULL; iter = g_slist_next(iter)) {
		...
		if (strncmp(g_quark_to_string(__alarm_info->quark_bundle), "null", 4) != 0) {
			...
			if (name_has_owner_reply == false && ret == PMINFO_R_OK) {
				...
				__alarm_send_noti_to_application(destination_app_service_name, alarm_id, __alarm_info->uid); /* dbus auto activation */
